...
const POWER_UP_DELAY_MS = 5000;
const POWER_UP_DURATION_MS = 15000;
const POWER_UP_FLICKER_WARNING_MS = 4000;
...
function createPowerUpCell(x, y) {
  return {
    x,
    y,
    rotation: Math.random() * Math.PI * 2,
...
function moveEnemySnake(enemySnake, now) {
  ...
  const nextDirection = safeDirections
    .map((directionOption) => ({
      direction: directionOption,
      score: getProjectedEnemyScore(enemySnake, directionOption),
    }))
...
function syncTimedEvents(elapsed) {
  ...
  if (!powerUp && !isPoweredUp(elapsed)) {
...
function moveSnake() {
  ...
  const hitEnemySnakes = enemySnakes.filter((enemySnake) =>
  ...
  if (isPoweredUp(elapsed) && hitObstacle) {
    obstacles = obstacles.filter((obstacle) => !cellsMatch(obstacle, nextHead));
  }

  if (isPoweredUp(elapsed) && hitEnemySnakes.length > 0) {
...
function drawSnake() {
  ...
  drawBody(snake, direction, theme);
  drawHead(snake, direction, theme);
}
...
function drawPoweredAura() {
  ...
  const points = snake.map(getCellCenter);
